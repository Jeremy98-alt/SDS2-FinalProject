...
- ***Logistic regression model:*** Logistic regression is the appropriate regression analysis to conduct when the dependent variable is dichotomous (binary). Logistic regression is used to describe data and to explain the relationship between one dependent binary variable and one or more nominal, ordinal, interval or ratio-level independent variables. This is more faster than other binary classificators.
...
summary(glm(y ~ x1+x2+x3+x4+x5+x6+x7+x8+x9+x10+x11+x12+x13, family = binomial(link = "logit"),data=dat))
```
...
logit(p_i) =  log\Big(\frac{p_i}{1-p_i}\Big)= \beta_{2} \cdot x_{2_i} + \beta_{3} \cdot x_{3_i} + \beta_{4} \cdot x_{4_i} +\beta_{8} \cdot x_{8_i} + \beta_{9} \cdot x_{9_i} + \beta_{10} \cdot x_{10_i} + \beta_{11} \cdot x_{11_i} + \beta_{12} \cdot x_{12_i} + \beta_{13} \cdot x_{13_i}
$$
...
$$
\beta_i \sim N(\mu = 0, \tau^2 = 0.000001)  \,\,\, for \,\, i \,\, = 2,3,4,8,9,..,13
$$
...
model <- function(){
  ...
  for (i in 1:N){
    y[i] ~ dbern(p[i])
    logit(p[i]) <-  beta0 + beta2*x2[i] + beta3*x3[i] + beta4*x4[i] + beta7*x7[i] + beta9*x9[i] + beta10*x10[i] + beta12*x12[i] + beta13*x13[i] # omit intercept and not useful variables
  }
...
data.jags <- list("y" = y, "N" = N,
                  "x2" = x2, "x3" = x3, "x4" = x4, "x7" = x7, "x9" = x9, "x10" = x10, "x12" = x12, "x13" = x13)

# Defining parameters of interest to show after running RJags
mod.params <- c("beta0", "beta2", "beta3", "beta4", "beta7", "beta9", "beta10", "beta12", "beta13")
```
...
bayesplot::mcmc_combo(chainArray, pars = c("beta4", "beta7", "beta9"))
bayesplot::mcmc_combo(chainArray, pars = c("beta10", "beta12", "beta13"))
```
  ...
  hchart('line', hcaes(x = 1:nrow(df), y = cummean(df[, 22])), color = "red", name = "First Chain") %>%
    hc_add_series( cummean(df[, 23]), type = "line", color = "blue", name = "Second Chain") %>%
      hc_add_series( cummean(df[, 24]), type = "line", color = "green", name = "Thrid Chain") %>%
        hc_title(text = "The Empirical Mean of beta7") %>%
          hc_xAxis(title = list(text = "Iteration")) %>%
            hc_yAxis(title = list(text = 'Cumulative Mean'))
...
df %>%
  hchart('line', hcaes(x = 1:nrow(df), y = cummean(df[, 28])), color = "red", name = "First Chain") %>%
    hc_add_series( cummean(df[, 29]), type = "line", color = "blue", name = "Second Chain") %>%
      hc_add_series( cummean(df[, 30]), type = "line", color = "green", name = "Thrid Chain") %>%
        hc_title(text = "The Empirical Mean of deviance") %>%
          hc_xAxis(title = list(text = "Iteration")) %>%
            hc_yAxis(title = list(text = 'Cumulative Mean'))
```

As we can see above each parameter achieves in all of the three chains generated the same end point, so means that with different initial points in these three chains, we are strictly going to have the same estimated mean parameter at the end.
...
Here, we want to understand if we achieve with these multiple simulations of the markov chains the convergences and the validity of the stationarity regions. We are considering different tools saw in the last lectures, to understand if we achieve or not the convergency, the tools utilized are:
...
$$~$$
This Introduces an MCMC diagnostic that estimates the number of iterations needed for a given level of precision in posterior samples, as well as estimating burn-in, when quantiles are the posterior summaries of interest.
...
We prefer considering more the Heidel test because it is a double check of the stationarity and the convergency states. This is sometimes failed, meanwhile the other tools seems to be good, so I prefer considering this test.
...
After checking that the simulations returned to pass the convergence, we can decide to pool up all the chains and does the credible interval and the point estimates for our beta values estimated:
...
It's relevant to conclude that all your parameters (and its variables) are significant to predict heart attacks (except for $\beta_4$)
...
Here, there is a brief practical example where we predict 302 new random observations:

```{r}
# creating false N observations, trying to predict them (creating a false test set)
dat_test[c(2,3,4,6,7,8)] <- lapply(dat_test[c(2,3,4,6,7,8)], function(x) c(scale(x))) # scaling for predicting better than before
...
beta_est <- list("beta0" = mod.fit$BUGSoutput$summary["beta0", "mean"],
                 ...
                 "beta7" = mod.fit$BUGSoutput$summary["beta7", "mean"],
                 "beta9" = mod.fit$BUGSoutput$summary["beta9", "mean"],
...
dat_test$output_predictive <- apply(dat_test, 1, function(x){
  ...
  o <- beta_est$beta0 + beta_est$beta2*x["sex"] + beta_est$beta3*x["cp"] + beta_est$beta4*x["trtbps"] + beta_est$beta7*x["restecg"] + beta_est$beta9*x["exng"] + beta_est$beta10*x["oldpeak"] + beta_est$beta12*x["caa"] + beta_est$beta13*x["thall"]
...
highchart() %>% 
  ...
  hc_title(text = "The Classificator") %>%
...
With this fake predictions we are be able to see something in terms of accuracy of the model, as we can see above!
...
model2 <- function(){
  ...
  for (i in 1:N){
    y[i] ~ dbern(p[i])
    cloglog(p[i]) <-  beta4*x4[i]^2 + beta5*x5[i] + beta6*x6[i] + beta7*x7[i] + beta8*x8[i] + beta13*x13[i] # omit intercept and not useful variables
  }
...
```{r echo=FALSE}
x_logits <- mod.fit$BUGSoutput$summary["beta0","mean"] + mod.fit$BUGSoutput$summary["beta2","mean"]*x2 + mod.fit$BUGSoutput$summary["beta3","mean"]*x3 + mod.fit$BUGSoutput$summary["beta4","mean"]*x4 + mod.fit$BUGSoutput$summary["beta7","mean"]*x7 + mod.fit$BUGSoutput$summary["beta9","mean"]*x9 + mod.fit$BUGSoutput$summary["beta10","mean"]*x10 + mod.fit$BUGSoutput$summary["beta12","mean"]*x12 + mod.fit$BUGSoutput$summary["beta13","mean"]*x13

x_probs <- 1/(1+exp(-x_logits))

df <- data.frame("x2" = sort(x2), "x3" = sort(x3), "x4" = sort(x4), "x7" = sort(x7),
                 "x9" = sort(x9), "x10" = sort(x10),
...
beta_7 <- mod.fit$BUGSoutput$summary["beta7","mean"]
beta_9 <- mod.fit$BUGSoutput$summary["beta9","mean"]
...
# Simulate the independent variables - TRY WITH THE SAME DATA X2, X3...
set.seed(123)
sim_dat <- data.frame("sim_x2" = sample(x2, N, replace=T),
                      "sim_x3" = sample(x3, N, replace=T),
                      "sim_x4" = sample(x4, N, replace=T),
                      "sim_x7" = sample(x7, N, replace=T),
                      "sim_x9" = sample(x9, N, replace=T),
...
linpred <- beta_0 + beta_2*sim_dat$sim_x2 + beta_3*sim_dat$sim_x3 + beta_4*sim_dat$sim_x4 + beta_7*sim_dat$sim_x7 + beta_9*sim_dat$sim_x9 + beta_10*sim_dat$sim_x10 + beta_12*sim_dat$sim_x12 + beta_13*sim_dat$sim_x13
...
sim_data.jags <- list("y" = sim_dat$output, "N" = N,
                  ...
                  "x4" = sim_dat$sim_x4, "x7" = sim_dat$sim_x7, 
                  "x9" = sim_dat$sim_x9, "x10" = sim_dat$sim_x10, 
...
```{r echo=FALSE}
comparison <- rbind(mod.fit$BUGSoutput$summary[, "mean"], mod.fit_sim$BUGSoutput$summary[, "mean"])
rownames(comparison) <- c("Estimated", "True")
