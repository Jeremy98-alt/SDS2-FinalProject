...
<center> <i> Statitics for Data Science and Laboratory II </i> </center>
<center> <b> "La Sapienza" University of Rome</b> </center>
...
- <a href="#conclusions"> The Conclusions </a>
    + <a href="#simulatedata"> Simulation for parameters recovery </a>
...
$$
\beta_i \sim N(\mu = 0, \tau^2 = 1.0^{-6})  \,\,\, for \,\, i \,\, = 2,3,4,8,9,..,13
$$
...
mod.fit <- jags(data = data.jags,                                                 # DATA
                ...
                n.chains = n.chains, n.iter = 10000, n.burnin = 1000)  # MCMC
...
Here, we want to understand if we achieve with these multiple simulations of the markov chains the convergences and the validity. We are considering different tools saw in the last lectures, to understand if we achieve or not the convergency, the tools utilized are:
...
$$~$$
Introduced an MCMC diagnostic that estimates the number of iterations needed for a given level of precision in posterior samples, as well as estimating burn-in, when quantiles are the posterior summaries of interest.
...
## <a id="simulatedata"> Simulation for parameters recovery </a>
...
beta_2 <- mod.fit$BUGSoutput$summary["beta2","mean"] # rnorm(1, mean = -1, sd = 0.000001) 
beta_3 <- mod.fit$BUGSoutput$summary["beta3","mean"] # rnorm(1, mean = 0.5, sd = 0.000001) 
beta_4 <- mod.fit$BUGSoutput$summary["beta4","mean"] # rnorm(1, mean = 0, sd = 0.000001) 
beta_8 <- mod.fit$BUGSoutput$summary["beta8","mean"] # rnorm(1, mean = 0, sd = 0.000001) 
beta_9 <- mod.fit$BUGSoutput$summary["beta9","mean"] # rnorm(1, mean = -0.5, sd = 0.000001) 
beta_10 <- mod.fit$BUGSoutput$summary["beta10","mean"] # rnorm(1, mean = 0.5, sd = 0.000001) 
beta_11 <- mod.fit$BUGSoutput$summary["beta11","mean"] # rnorm(1, mean = 0.5, sd = 0.000001) 
beta_12 <- mod.fit$BUGSoutput$summary["beta12","mean"] # rnorm(1, mean = -0.5, sd = 0.000001) 
beta_13 <- mod.fit$BUGSoutput$summary["beta13","mean"] # rnorm(1, mean = -0.5, sd = 0.000001) 

# Simulate the independent variables - TRY WITH THE SAME DATA X2, X3...
set.seed(123)
sim_dat <- data.frame("sim_x2" = rbinom(N, 0:1, prob = mean(x2)),
                      "sim_x3" = rbinom(N, 0:3, prob = mean(x3)),
                      "sim_x4" = round(rnorm(N, mean = mean(x4), sd = sd(x4))),
                      "sim_x8" = round(rnorm(N, mean = mean(x8), sd = sd(x8))),
                      "sim_x9" = rbinom(N, 0:1, prob = mean(x9)),
                      "sim_x10" = round(rnorm(N, mean = mean(x10), sd = sd(x10)),1),
                      "sim_x11" = rbinom(N, 0:2, prob = mean(x11)-1),
                      "sim_x12" = rbinom(N, 0:3, prob = mean(x12)),
                      "sim_x13" = rbinom(N, 0:3, prob = mean(x13)-2)
          )

# Simulate response according to the model
linpred <- beta_2*sim_dat$sim_x2 + beta_3*sim_dat$sim_x3 + beta_4*sim_dat$sim_x4 + beta_8*sim_dat$sim_x8 + beta_9*sim_dat$sim_x9 + beta_10*sim_dat$sim_x10 + beta_11*sim_dat$sim_x11 + beta_12*sim_dat$sim_x12 + beta_13*sim_dat$sim_x13
...
sim_data.jags <- list("y" = sim_dat$output, "N" = N,
                  ...
                  "x4" = sim_dat$sim_x4, "x8" = sim_dat$sim_x8, 
                  "x9" = sim_dat$sim_x9, "x10" = sim_dat$sim_x10, 
                  "x11" = sim_dat$sim_x11, "x12" = sim_dat$sim_x12, 
                  "x13" = sim_dat$sim_x13)
